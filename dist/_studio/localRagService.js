...
// --- Semantic Chunking ---
function chunkText(text, targetSize = 600, overlap = 50) {
    const chunks = [];
    let index = 0;
    text = text.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
...
export const retrieveLivedExperience = async (agentId, query) => {
    try {
...
// --- Public API ---
export const initLocalRAG = async () => {
    try {
        await db.open();
...
export const addDocument = async (projectId, entry) => {
...
export const deleteDocumentsByLoreEntryId = async (loreEntryId, projectId) => {
    const allDocsInProject = await db.getAll(STORE_NAME, 'projectId', projectId);
    const docsToDelete = allDocsInProject.filter(doc => doc.loreEntryId === loreEntryId);
    for (const doc of docsToDelete) {
        await db.delete(STORE_NAME, doc.id);
    }
};
export const clearProjectDocuments = async (projectId) => {
    const allDocsInProject = await db.getAll(STORE_NAME, 'projectId', projectId);
...
export const searchDocuments = async (projectId, query, limit = 5) => {
...
export const getAllDocuments = async (projectId) => {
    const allDocs = await db.getAll(STORE_NAME, 'projectId', projectId);
...
export const clearAllLocalRAG = async () => {
