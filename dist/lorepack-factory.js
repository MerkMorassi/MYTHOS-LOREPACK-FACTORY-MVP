...
export class LorepackFactory {
    ...
    async ingestBatches(batches, options) {
        const batchSize = 40;
        const concurrency = 5;
        let processed = 0;
        let written = 0;
        const groups = Array.from({ length: Math.ceil(batches.length / batchSize) }, (_, index) => batches.slice(index * batchSize, (index + 1) * batchSize));
    ...
    async ingestConversationalTurn(agentId, agentHandle, userText, modelText) {
    ...
    async *yieldExportBatches(agentId, batchSize = 1000) {
    ...
    async importLorepack(file, agentId, onProgress) {
        ...
        const consume = async (line) => {
            ...
            if (vectors.length + edges.length >= 500)
                await flush();
        };
        while (true) {
    ...
    async syncToEndpoint(endpoint, cursor) {
        ...
        for (let index = 0; index < pending.length; index += 500) {
            ...
            if (!response.ok)
                throw new Error(`LorePack sync failed: ${response.status}`);
        }
